...
class QLearner():
    ...
    def __init__(self, actions, Q_init=13, gamma=0.8):
    ...
    def next_action(self, state):
        Qtable = self.QTable
        if Qtable.has_key(state) :  #check if state has been encountered before or not
            ...
            if random.random() < self.epsilon :
                ...
                argmax_actions = {action:Qhat for action, Qhat in Qtable[state].items() \
                                    if Qhat == max(Qtable[state].values())}
    ...
    def update(self, state, previous, steps_count):
        ...
        if steps_count > 0 :  #make sure it is not the first step in a trial.
            ...
            Q_hat = Q_hat + (self.alpha * (previous['reward'] + (self.gamma * (max(Qtable[state].values()))) - Q_hat))
